...
Para visualizar y ejecutar el pipeline, puede optar por cualquiera de las siguientes alternativas:

1. **VS Code (Recomendado)**: Utilizando la extensión oficial de **Jupyter** para una experiencia integrada directamente en el IDE.
2. **Jupyter Notebook Clásico**: Levantando el servidor local desde la terminal mediante el comando `jupyter notebook`.
...
**Nota Técnica**: El sistema implementa técnicas de prevención de _Data Leakage_ mediante el uso de `StandardScaler` ajustado únicamente sobre el conjunto de entrenamiento.
